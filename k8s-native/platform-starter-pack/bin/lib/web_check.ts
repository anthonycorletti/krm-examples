...
try {
  ...
  await wait("!!document.querySelector('.sidebar')");
  const lifetime = await evaluate(`(() => {
    const raw = localStorage.getItem('platform.access-token');
    const payload = JSON.parse(atob(raw.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')));
    return payload.exp - payload.iat;
  })()`);
  if (lifetime < 86340 || lifetime > 86400)
    throw new Error(`Unexpected token lifetime: ${lifetime}`);
  console.log(
    "Sign-in survived a page reload; access token lifetime is 24 hours.",
  );
  ...
  await evaluate(
    `document.querySelector('a[href="/?view=warehouse"]').click()`,
  );
  ...
  await evaluate("document.querySelector('button[value=disconnect]').click()");
  await wait("location.pathname === '/connect'");
  if (await evaluate("!!localStorage.getItem('platform.access-token')"))
    throw new Error("Sign-out retained the access token");
  console.log("Sign-out cleared persisted credentials.");
  await evaluate(
    `localStorage.setItem('platform.access-token', 'e30.' + btoa(JSON.stringify({ exp: Math.floor(Date.now() / 1000) - 1 })) + '.test')`,
  );
  await send("Page.reload");
  await wait("!localStorage.getItem('platform.access-token')");
  console.log("Expired persisted tokens are discarded.");
  if (errors.length)
