...
The full local platform runs on a local K3s cluster. The current tooling uses the default Colima profile with Kubernetes enabled and the Docker runtime. It uses the same KRM bases across supported Kubernetes distributions, including Argo, with one Postgres instance and local-path persistent storage. No kind or cloud application service is required.

## Local K3s setup

The platform runs on a local K3s cluster. Colima is the VM host used to run K3s for this example; it is not an application or platform dependency. The working setup uses Colima's `default` profile and the Kubernetes context named `colima`.

The included convenience scripts target that setup for cluster access, image loading, and control-plane administration. Those tooling defaults are separate from the Kubernetes manifests. No setup changes are needed to keep using the demonstrated environment.
