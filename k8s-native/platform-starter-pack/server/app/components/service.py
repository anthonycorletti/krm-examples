...
class ComponentsService:
    ...
    async def list(self, principal, environment) -> list[ComponentRead]:
        ...
        if settings.object_endpoint:
            ...
            await self.encryption_evidence(components["secrets"], settings.workflow_namespace)
    ...
    async def encryption_evidence(self, component, namespace):
        try:
            resource = await get_resource(
                f"/api/v1/namespaces/{namespace}/configmaps/platform-encryption-evidence"
            )
            data = resource["data"]
            ...
            if (
                ...
                and "Encryption Status: Enabled" in status
                and "reencrypt_finished" in status
                and "All hashes match" in status
            ):
                component.status = "ready"
                component.detail = (
                    f"K3s control-plane check at {verified.isoformat()}: encryption enabled; "
                    "existing Secrets reencrypted; server hashes match. "
                    "Administrator attestation, valid 24h."
                )
            else:
                component.status = "unverified"
                component.detail = (
                    "Encryption evidence is stale or incomplete; run bin/local-secrets-check."
                )
        except (httpx.HTTPError, OSError, ValueError, KeyError, TypeError):
            component.status = "unverified"
            component.detail = (
                "No current control-plane encryption evidence; run bin/local-secrets-check."
            )
