...
The local K3s cluster uses Keycloak OIDC tokens from browser PKCE or bin/local-token. Native local/test development supports short-lived symmetric tokens from bin/dev-token. Remote environments require configured OIDC issuer/JWKS validation; browser PKCE and MCP discovery support are present, with remote-provider validation still pending. Verify with bin/server-check and bin/integration-test.
