...
An experimentation workspace for giving agents tasks, following up with instructions, and inspecting the platform that executes them. The application runs on portable Kubernetes resources; EKS is the cloud deployment target and an existing Colima Kubernetes cluster runs the full local stack. No kind.
