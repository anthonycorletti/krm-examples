...
Pinned release: [kubernetes-sigs/gateway-api v1.6.2](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.6.2). upstream.yaml is the unmodified release asset; its SHA-256 matches GitHub release metadata. LICENSE is the Apache-2.0 notice copied from the release tag. Exact URLs and checksums are in sources.txt. Signature verification and a complete container-image license inventory remain pending.

Resource ownership: Gateway API standard-channel CRDs and validation resources; no controller.

Run bin/vendor-check to verify copies, or bin/vendor-fetch gateway-api to restore these exact versions. To upgrade, review the upstream release and compatibility requirements, update release URLs/checksums and this README, fetch the pinned files, review the CRD/RBAC/image diff, and run bin/krm-check. Keep deployment overrides under cluster/; do not edit upstream.yaml. Validate the composed configuration on EKS before promotion.
